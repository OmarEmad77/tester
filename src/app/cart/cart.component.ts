...
export class CartComponent implements OnInit {
  ...
  // وظيفة لحذف منتج من العربة
  removeFromCart(item: any) {
    this._cartService.removeFromCart(item); 
    this.calculateTotalPrice(); 
  }

  // وظيفة لتفريغ الـ cart
  clearCart() {
  ...
  // وظيفة لتحديث الكمية عند تغييرها
  updateQuantity(item: any, quantity: number) {
    const foundItem = this.cartItems.find(cartItem => cartItem.id === item.id);
    if (foundItem) {
      foundItem.quantity = quantity;
      this.calculateTotalPrice(); // إعادة حساب السعر الإجمالي بعد التحديث
    }
  }
}
